  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 380px", gap: 12 }}>
      <Panel
        ...
        subtitle={`/var/agentic/deploys/raas/2026.05.16-a/${agent.name}.ts`}
        padded={false}
        action={<div style={{ display: "flex", gap: 6 }}>
          <Button small icon="external" tone="ghost">Open in IDE</Button>
          <Button small icon="upload" tone="ghost">Download</Button>
        </div>}
      >
        <window.MonacoEditor
          value={code}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <Panel title="ontology_instructions" subtitle="Domain vocabulary & rules handed to the LLM" padded={false}>
          <div style={{
            padding: "10px 14px",
        ...
        <Panel title="input_data" subtitle="Sample input for testing & doc generation" padded={false}>
          <window.MonacoEditor
        ...
        <Panel title={`tool_use · ${tools.length}`} subtitle="Tool API surface exposed to the LLM" padded={false}>
          <div>
            {tools.map((t) => (
