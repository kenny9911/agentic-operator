...
function DeployAgentModal({ onClose }) {
  ...
  return (
    <ModalOverlayA onClose={onClose}>
      <div style={{ width: 900, maxHeight: "88vh", background: "var(--panel)", border: "1px solid var(--border-2)", borderRadius: 8, overflow: "hidden", boxShadow: "0 24px 60px -20px rgba(0,0,0,0.6)", display: "flex", flexDirection: "column" }}>
        {/* Header + stepper */}
        ...
        <div style={{ padding: 20, overflow: "auto", flex: 1, minHeight: 0 }}>
          ...
          {step === 3 && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {/* Model + prompt */}
              <div>
                <EditField label="Model" hint="Pick from the fleet, or rely on the workflow default.">
                  <select value={model} onChange={e => setModel(e.target.value)} style={editSelectStyle}>
                    <option>claude-sonnet-4-5</option>
                    <option>claude-haiku-4-5</option>
                    <option>gpt-4.1-mini</option>
                ...
                <EditField label="System prompt" hint="Use {{vars}} to interpolate run context.">
                  <EditTextarea
              ...
              {/* Tools */}
              <div>
                <EditField label={`Tools · ${tools.length} selected`} hint="Tool bindings this agent may call. Add more from the workspace catalog.">
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 360, overflow: "auto", padding: 2 }}>
                    {COMMON_TOOLS.map(t => {
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div>
                <Panel title="Pre-flight" padded>
                  ...
                  <ValidationLine ok label={`${tools.length} tools wired`} />
