...
const { useState: useStateAg, useMemo: useMemoAg, useEffect: useEffectAg } = React;

// Force re-render when runs or events change (e.g. window.testAgent emits).
function useLiveData() {
  const [, tick] = useStateAg(0);
  useEffectAg(() => {
...
function Agents({ navigate, params, models = [] }) {
  useLiveData();
  ...
  const [listW, setListW] = useStateAg(440);
  const selectedId = params.agentId;

  const stats = useMemoAg(() => {
    const m = new Map();
    agents.forEach(a => m.set(a.id, { runs: 0, errors: 0, lastRun: 0, tests: 0, lastTestRunId: null, lastTestAt: 0 }));
    runs.forEach(r => {
      const s = m.get(r.agentId);
      if (!s) return;
      s.runs++;
      if (r.status === "failed") s.errors++;
      if (r.startedAt > s.lastRun) s.lastRun = r.startedAt;
      if (r.testRun) {
        s.tests++;
        if (r.startedAt > s.lastTestAt) { s.lastTestAt = r.startedAt; s.lastTestRunId = r.id; }
      }
    });
    return m;
  }, [agents, runs]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "row", minHeight: 0 }}>
        ...
        {/* Splitter + Detail (only when an agent is selected) */}
        {selectedId && (
          <>
            <window.Splitter axis="x"
              getValue={() => listW} setValue={setListW}
              min={260} max={720} />
            <div style={{ flex: 1, minWidth: 0, overflow: "auto", minHeight: 0 }}>
              <AgentDetail agent={agents.find(a => a.id === selectedId)} stats={stats.get(selectedId)} navigate={navigate} models={models} />
            </div>
          </>
        )}
      </div>

      {deployOpen && <DeployAgentModal onClose={() => setDeployOpen(false)} models={models} />}
...
function AgentDetail({ agent, stats, navigate, models = [] }) {
    ...
    <div style={{ padding: 24, height: "100%", display: "flex", flexDirection: "column", minHeight: 0 }}>
      <header style={{ marginBottom: 16, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          ...
          {lastTest && (
            <button
              onClick={() => navigate("runs", { runId: lastTest.id })}
            ...
            >
              <StatusDot status={lastTest.status} size={6} />
              TEST · {window.fmtAgo(lastTest.startedAt)}
            </button>
          )}
          <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
            ...
            {editing
              ...
              : <>
                  ...
                  <Button small icon="run" tone="primary" onClick={() => {
                    const run = window.testAgent(agent);
                    if (run) navigate("runs", { runId: run.id });
                  }}>Test run</Button>
                </>
            }
          </div>
      ...
      {/* Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, border: "1px solid var(--border)", borderRadius: 6, background: "var(--panel)", marginBottom: 16, flexShrink: 0 }}>
      ...
      {/* Tab content fills remaining height. Code tab fills directly; others scroll within. */}
      <div style={{ flex: 1, minHeight: 0, overflow: tab === "code" ? "hidden" : "auto" }}>
        {tab === "config" && (editing ? <EditConfigTab agent={agent} models={models} /> : <ConfigTab agent={agent} />)}
...
// =====================================================
function EditConfigTab({ agent, models = [] }) {
  ...
  const [model, setModel] = useStateAg(agent.model || models[0]?.name || "");
...
function DeployAgentModal({ onClose, models = [] }) {
  ...
  const [model, setModel] = useStateAg(models[0]?.name || "");
