...
function App() {
  ...
  // up-to-date list.
  useEffectApp(() => {
    window.RAAS_SETTINGS_MODELS = models;
  }, [models]);
  ...
  // Reload bootstrap data when the chosen data source changes.
  useEffectApp(() => {
    if (!window.RAAS_RELOAD) return;
    if (tweaks.dataSource === window.RAAS_DATA_SOURCE) return;
    window.RAAS_RELOAD(tweaks.dataSource);
  }, [tweaks.dataSource]);
...
// ─── Boot wrapper: waits on window.RAAS_BOOTSTRAP before rendering App ───────
function Boot() {
  const [state, setState] = useStateApp(() => {
    if (window.RAAS_BOOTSTRAP_LOADED_AT) return { kind: "ready" };
    if (window.RAAS_BOOTSTRAP_ERROR)
      return { kind: "error", message: window.RAAS_BOOTSTRAP_ERROR };
    return { kind: "loading" };
  });

  useEffectApp(() => {
    const onLoad = () => setState({ kind: "ready" });
    const onError = (e) =>
      setState({ kind: "error", message: e.detail || "bootstrap failed" });
    window.addEventListener("raas-data-loaded", onLoad);
    window.addEventListener("raas-data-error", onError);
    return () => {
      window.removeEventListener("raas-data-loaded", onLoad);
      window.removeEventListener("raas-data-error", onError);
    };
  }, []);

  if (state.kind === "ready") return <App />;
...
function BootSplash() {
  return (
    <div style={{
      ...
      gap: 12, fontFamily: "var(--mono)", fontSize: 11.5, letterSpacing: "0.08em",
    }}>
      <span style={{
...
function BootError({ message }) {
  const reload = () => window.RAAS_RELOAD && window.RAAS_RELOAD("json");
  return (
    <div style={{
      height: "100%",
      display: "flex", flexDirection: "column",
      ...
      }}>BOOTSTRAP FAILED</div>
      <div style={{ fontSize: 13, color: "var(--text-2)", maxWidth: 520 }}>
        {message}
      </div>
      <button
        onClick={reload}
        style={{
          ...
          border: "1px solid var(--signal)", borderRadius: 5,
          fontFamily: "var(--sans)", fontSize: 12, fontWeight: 500,
        }}>
        Retry with JSON source
      </button>
    </div>
  );
}
