...
import { SETTINGS_MEMBERS } from "../data";
...
export function MembersSection() {
  ...
  const rows = SETTINGS_MEMBERS.filter((m) => {
    if (filter === "service" && m.role !== "Service") return false;
    if (filter === "human" && m.role === "Service") return false;
    if (
      q &&
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      >
        {[
          { role: "Owner" as const,    perms: "Everything · billing · destroy" },
          { role: "Admin" as const,    perms: "Deploy · members · keys" },
          { role: "Operator" as const, perms: "Run · approve tasks · view" },
          { role: "Viewer" as const,   perms: "Read-only across workspace" },
          { role: "Service" as const,  perms: "Machine accounts (CI, bots)" },
        ].map((r) => (
          <div
            key={r.role}
      ...
      <Panel title="SSO & domain" padded>
        <Field
          label="SSO provider"
          hint="All non-service members must sign in via SSO."
        >
          <SelectIn
            value="Okta SAML 2.0"
            options={[
        ...
        <Field
          ...
          hint="Sign out inactive sessions automatically."
        >
          <SelectIn
            value="8 hours"
            options={[
