...
const PROMPT_PREAMBLE = {
  collectChainExecutionData: [
    ...
    "2）入参一律用 metaERP 的小驼峰命名（pbpNumberList、itemCodeList、prCreationDateFrom），",
    "   字段名照抄工具描述里该操作的入参清单，不要用本体里的下划线写法。",
    "3）**本轮只处理平台锁定的那一条采购计划**。queryOpenPbpHeader 不带单号时平台会自动",
    "   锁定到这一单，你不要自己编计划编号，也不要去查别的计划。",
    "4）**每个操作最多调用一次**（queryAwardList 之类按 ID 展开的也一样）：拿到结果后一次性",
    "   传全部 ID 的列表，不要对查到的每条询价单/定标结果再单独发一轮。整个运行的 ERP 调用",
    "   有硬上限，超了会直接失败——扇出失控比查得不全更糟。",
    "",
