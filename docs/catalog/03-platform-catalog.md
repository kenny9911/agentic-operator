...
### PF-CI-01 — `ci.yml` ✅
Triggers: push to `main`, PR targeting `main`, manual dispatch. Concurrency group `ci-${{ github.ref }}` with `cancel-in-progress: true` so force-pushes to a PR branch don't burn CI minutes. Env pins: `NODE_VERSION=26.8.1`, `PNPM_VERSION=11.21.0`, `LLM_DEFAULT_PROVIDER=mock`, `LLM_DEFAULT_MODEL=mock-model-v1`, `NODE_ENV=test`. Jobs (DAG):
