...
The contract is sealed at `BaseAgent.run()` — subclasses cannot override the entry point; they only override the hooks. `run()` delegates to `executeAgentRun(this, input, ctx)` in `packages/agent-runtime/src/run-engine.ts`, which owns the entire run-row + step-row + file-log + gateway dispatch lifecycle. This is the single most important architectural invariant on the agent side: a code agent never reaches into the DB, never writes its own log line, never decides whether to retry — those are run-engine responsibilities. See `packages/agent-runtime/src/base-agent.ts:35-108` for the abstract surface; `packages/agents/src/base-agent.ts:29-78` is the legacy mirror still consumed by `apps/api` (`@agentic/agents` is the API entry point, with the newer `@agentic/agent-runtime` planned to supersede it — see `CLAUDE.md` § Workspace package map for the migration intent).
...
- `setAgentGateway(getLLMGateway())` in `packages/agents/src/gateway-host.ts` and `packages/agent-runtime/src/gateway-host.ts` — `BaseAgent`'s run engine reads it via the gateway host.
